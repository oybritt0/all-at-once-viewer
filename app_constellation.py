...
<style>
  ...
  #overlay input[type=text], #overlay input:not([type=range]) { user-select: text; }
...
<div id="overlay">
  ...
  <hr>
  <div class="row"><b>match worlds view</b> <span style="color:#5c6270;font-size:10px;">paste from original viewer</span><br>
    <input id="camIn" placeholder="paste camera string" style="width:210px;font-size:10px;background:#11131a;color:#cdd2dc;border:1px solid #3a3f4b;border-radius:3px;padding:1px 3px;"><br>
    <button id="applyCam" style="margin-top:3px;">apply view</button>
    <span id="camMsg" style="color:#5c6270;font-size:10px;margin-left:4px;"></span></div>
  <hr>
  <div class="row"><b>export</b>
    <button id="exportSVG">SVG</button>
    <button id="exportPNG">PNG</button></div>
  <div class="row" style="font-size:10px;color:#9aa3b5;">match size (= worlds SVG)<br>
    <input id="expW" type="number" value="1600" min="200" max="8000" step="50" style="width:52px;background:#11131a;color:#cdd2dc;border:1px solid #3a3f4b;border-radius:3px;"> &times;
    <input id="expH" type="number" value="1200" min="200" max="8000" step="50" style="width:52px;background:#11131a;color:#cdd2dc;border:1px solid #3a3f4b;border-radius:3px;"><br>
    <button id="exportPNGmatch" style="margin-top:3px;">PNG match</button>
    <button id="exportSVGmatch">SVG match</button><br>
    <span style="color:#5c6270;">same dims + aspect as the worlds SVG &mdash; drops straight over it. set by paste, or type to match.</span></div>
</div>
<div id="tooltip"></div>
<div id="vrhint"></div>
...
// Match worlds view: paste the camera string copied from the original viewer's
// 3D overlay and reproduce that exact perspective here. Both viewers share the
// same raw UMAP coordinates and centroid framing, so position + target + fov
// transfer directly (desktop only; the dolly/world groups are identity here).
// After applying, use the PNG button to save the matching image.
function applyCamFromString() {
  const msg = document.getElementById('camMsg');
  const setMsg = (t) => { if (msg) { msg.textContent = t; if (t) setTimeout(()=>{ if(msg.textContent===t) msg.textContent=''; }, 2200); } };
  let c;
  try { c = JSON.parse((document.getElementById('camIn').value || '').trim()); }
  catch (e) { setMsg('could not read string'); return; }
  if (!c || !Array.isArray(c.pos) || c.pos.length < 3 || !Array.isArray(c.tgt) || c.tgt.length < 3) {
    setMsg('not a camera string'); return;
  }
  camera.position.set(+c.pos[0], +c.pos[1], +c.pos[2]);
  controls.target.set(+c.tgt[0], +c.tgt[1], +c.tgt[2]);
  if (typeof c.fov === 'number' && isFinite(c.fov)) {
    camera.fov = c.fov;
    uniforms.uFovScale.value = 1.0 / Math.tan(c.fov*Math.PI/360);
    if (lensEl)  lensEl.value = c.fov;
    if (lensVal) lensVal.textContent = c.fov.toFixed(0)+'\u00B0';
  }
  camera.updateProjectionMatrix();
  controls.update();
  unifyDirty = true;   // angular reach changed -> reform the worlds
  if (c.w && c.h) {
    const ew = document.getElementById('expW'), eh = document.getElementById('expH');
    if (ew) ew.value = c.w;
    if (eh) eh.value = c.h;
  }
  setMsg('matched \u2014 now export PNG');
}
document.getElementById('applyCam').onclick = applyCamFromString;
document.getElementById('camIn').addEventListener('keydown', (e) => {
  if (e.key === 'Enter') { e.preventDefault(); applyCamFromString(); }
});
...
// --- matched-size export ---------------------------------------------------
// Render at a fixed pixel size with the projection aspect set to that size, so
// the output overlays the worlds-view SVG exactly (same camera, same fov, same
// aspect, same dimensions). The on-screen view is restored afterwards via the
// normal resize path. These are separate from the live-size buttons above.
function matchDims(){
  const ew = parseInt(document.getElementById('expW').value, 10);
  const eh = parseInt(document.getElementById('expH').value, 10);
  return { EW: Math.max(50, ew || 1600), EH: Math.max(50, eh || 1200) };
}
function renderMatchedToCanvas(EW, EH){
  const prevRatio = renderer.getPixelRatio();
  camera.aspect = EW / EH; camera.updateProjectionMatrix();
  renderer.setPixelRatio(1); renderer.setSize(EW, EH, false);
  uniforms.uH.value = EH;
  renderer.render(scene, camera);
  const c = document.createElement('canvas'); c.width = EW; c.height = EH;
  const g = c.getContext('2d');
  g.fillStyle = '__BG__'; g.fillRect(0, 0, EW, EH);
  g.drawImage(renderer.domElement, 0, 0, EW, EH);
  renderer.setPixelRatio(prevRatio);
  onResize();                 // restore live aspect / size / uH
  renderer.render(scene, camera);
  return c;
}
function buildMatchedSVG(EW, EH){
  const prevAspect = camera.aspect;
  camera.aspect = EW / EH; camera.updateProjectionMatrix();
  const { proj, refDist } = projectScreen(EW, EH);
  let s = '<svg xmlns="http://www.w3.org/2000/svg" width="' + EW + '" height="' + EH +
          '" viewBox="0 0 ' + EW + ' ' + EH + '">';
  s += '<rect width="100%" height="100%" fill="__BG__"/>';
  if (lineSeg && lineSeg.visible){
    s += '<g stroke-opacity="' + lineSeg.material.opacity.toFixed(2) + '" stroke-width="0.6">';
    for (let e = 0; e < EDGES.length; e++){
      const a = EDGES[e][0], b = EDGES[e][1];
      if (aVis[a] < 0.5 || aVis[b] < 0.5) continue;
      const pa = proj[a], pb = proj[b]; if (!pa || !pb) continue;
  ...
  s += '<g>';
  for (let i = 0; i < N; i++){
    const pr = proj[i]; if (!pr) continue;
    const p = POINTS[i]; const b = Math.max(0.2, p.d);
    let r = (0.9 + b * 0.8) * (refDist / pr.depth) * 1.2; r = Math.max(0.4, Math.min(6, r));
    s += '<circle cx="' + pr.x.toFixed(1) + '" cy="' + pr.y.toFixed(1) +
         '" r="' + r.toFixed(2) + '" fill="' + rgbOf(i) +
         '" fill-opacity="' + Math.min(1, 0.4 + b * 0.6).toFixed(2) + '"/>';
  }
  s += '</g></svg>';
  camera.aspect = prevAspect; camera.updateProjectionMatrix(); controls.update();
  return s;
}
document.getElementById('exportPNGmatch').onclick = ()=>{
  const { EW, EH } = matchDims();
  const c = renderMatchedToCanvas(EW, EH);
  c.toBlob(b => downloadBlob(b, 'constellation_' + EW + 'x' + EH + '.png'));
};
document.getElementById('exportSVGmatch').onclick = ()=>{
  const { EW, EH } = matchDims();
  const s = buildMatchedSVG(EW, EH);
  downloadBlob(new Blob([s], {type:'image/svg+xml'}), 'constellation_' + EW + 'x' + EH + '.svg');
};
