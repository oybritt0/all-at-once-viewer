...
<div id="overlay">
  ...
  <div id="lineworkControls" class="row" style="display:none;margin-top:4px;color:#999;font-size:10px;">
    ...
    edge detail: <input id="edgeDetailRange" type="range" min="0.3" max="2.5" step="0.1" value="0.8" style="width:80px">
    <span id="edgeDetailValue">0.8</span><br>
    <span style="color:#666;font-size:9px;">(edge maps + worlds exemplars are traced from the high-res source image at load time — lower "edge detail" sigma = finer interior linework, recognizable features surface in worlds mode)</span><br>
...
let edgeDetailSigma = 0.8;            // Gaussian pre-blur sigma applied to each
                                       // image's full-resolution source before
                                       // Sobel. Lower = sharper, more interior
                                       // detail (mullions, figures, drawn
                                       // linework, texture) survives into both
                                       // the edge map AND the worlds-mode
                                       // content polygons / exemplars.
...
document.getElementById('edgeDetailRange').addEventListener('change', (e) => {
  edgeDetailSigma = parseFloat(e.target.value);
  document.getElementById('edgeDetailValue').textContent = edgeDetailSigma.toFixed(1);
  // Pre-blur sigma feeds into the per-image Sobel pass at edge-map /
  // content-polygon build time, so changing it requires rebuilding those
  // caches from the high-res source images, plus the worlds exemplar cache
  // (which is derived from contentPolygons) and the complexity ranking.
  _worldShapes = { key: null, shapes: null };
  precomputeAllEdgeMaps();
});
document.getElementById('edgeDetailRange').addEventListener('input', (e) => {
  document.getElementById('edgeDetailValue').textContent = parseFloat(e.target.value).toFixed(1);
});
    ...
    if (contentPolygons.size > 0) {
      ...
      if (worldsUnify > 0.001 && unions.length) {
        ...
        for (const wd of worldList) {
          ...
          // Recognizable interior linework, placed INSIDE each of this
          // world's union figures (not at the medoid's own, now-disconnected
          // position) so the lines you'd recognize from the source image in
          // images mode stay visually coincident with the welded blob in
          // worlds mode.
          ...
          if (md && md.inner && md.inner.length) {
            ...
            for (const g of wd.figs) {
              const bbox = contoursBBox(g.contours);
              const placed = remapInnerToBBox(md.inner, bbox, 0.9);
              for (const c of placed) {
                if (!c || c.length < 2) continue;
                const ip = [`M ${c[0][0].toFixed(2)},${c[0][1].toFixed(2)}`];
                for (let i = 1; i < c.length; i++) ip.push(`L ${c[i][0].toFixed(2)},${c[i][1].toFixed(2)}`);
  ...
  if (lineworkStyle === 'segmentation') {
    ...
    if (contentPolygons.size === 0) {
      ...
      if (worldsUnify > 0.001 && unions.length) {
        ...
        // Recognizable interior linework, placed INSIDE each union figure's
        // own bbox (not the medoid's own disconnected cx/cy/size) so the
        // lines stay visually coincident with the welded blob.
        const medoidByWorldCanvas = new Map();
        for (const f of forms) { if (f.isMedoid) medoidByWorldCanvas.set(f.lab, f); }
        for (const u of unions) {
          const md = medoidByWorldCanvas.get(u.lab);
          if (!md || !md.inner || !md.inner.length) continue;
          const dw = worldsDepthWeight(md.depth, depthRange, lineworkWeight);
          ctx.strokeStyle = dw.stroke; ctx.lineWidth = Math.max(0.18, dw.w * 0.45);
          const bbox = contoursBBox(u.contours);
          const placed = remapInnerToBBox(md.inner, bbox, 0.9);
          for (const c of placed) {
            if (!c || c.length < 2) continue;
            ctx.beginPath();
...
let edgeMapsRequested = false;        // true once precomputeAllEdgeMaps has run at least once
...
// Bounding box of a set of contours (each [[x,y],...] in screen coords).
// Used to find where a union figure actually sits so a medoid's recognizable
// interior lines can be placed inside it, rather than at the medoid's own
// (now-disconnected) position.
function contoursBBox(contours) {
  ...
  return { xmn, xmx, ymn, ymx, cx: (xmn+xmx)/2, cy: (ymn+ymx)/2, w: xmx-xmn, h: ymx-ymn };
}

// Remap a medoid's inner contours (image-relative [0,1]x[0,1] coords) into a
// target screen bbox, preserving the contours' own aspect ratio and centering
// them within the bbox (contain-fit, like object-fit: contain). Returns an
// array of contours in screen coords, or [] if inputs are degenerate.
function remapInnerToBBox(innerContours, bbox, fitFrac) {
  if (!bbox || !innerContours || !innerContours.length) return [];
  const ff = (fitFrac == null) ? 0.9 : fitFrac;   // inset slightly from the figure's edge
  const side = Math.min(bbox.w, bbox.h) * ff;
  const out = [];
  for (const c of innerContours) {
    if (!c || c.length < 2) continue;
    out.push(c.map(p => [
      bbox.cx + (p[0] - 0.5) * side,
      bbox.cy + (p[1] - 0.5) * side,
    ]));
  }
  return out;
}
...
function computeWorlds(W, H) {
  ...
  for (let i = 0; i < POINTS.length; i++) {
    ...
    // Medoid exemplars carry many more interior contours when edge detail
    // is set fine, so the recognizable linework of the source image shows
    // through at exemplar scale. Secondary (non-medoid) forms stay light
    // (just outline + a couple of inner lines) to avoid clutter.
    const innerCap = isMedoid
      ? Math.round(3 + 21 * Math.min(1, Math.max(0, (1.5 - edgeDetailSigma) / 1.2)))
      : 2;
    forms.push({
      cx: pr.x, cy: pr.y, size, depth: pr.depth, lab, idx: i, isMedoid,
      dom: polys[0], full: isMedoid ? polys : null, inner: polys.slice(1, 1 + innerCap),
...
function extractContentPolygonsFromGrad(grad, w, h) {
  ...
  //
  // Detail scaling: the contour level, simplification tolerance, and the
  // number of contours retained all move with edgeDetailSigma and the
  // source image's own resolution. A low edgeDetailSigma (sharp) on a
  // high-res source should let many MORE, FINER contours through — these
  // become the medoid's "inner" lines in worlds mode, i.e. the recognizable
  // interior linework (mullions, structure, drawn detail) rather than just
  // 1-3 coarse blob outlines.
  let maxG = 0;
  for (let i = 0; i < grad.length; i++) if (grad[i] > maxG) maxG = grad[i];
  if (maxG < 1) return [];
  // Lower edgeDetailSigma -> lower level fraction -> more, fainter contours
  // captured. Maps sigma 0.3..2.5 to level fraction ~0.12..0.30.
  const levelFrac = 0.12 + 0.18 * Math.min(1, Math.max(0, (edgeDetailSigma - 0.3) / 2.2));
  const level = levelFrac * maxG;
  const segs = marchingSquaresAt(grad, w, h, level, 1, 1);
  const paths = stitchSegmentsToPaths(segs, 0.5);
  const margin = Math.max(3, Math.round(Math.min(w, h) * 0.07));
  // Simplification tolerance scales with source resolution so a 256px+
  // source isn't flattened by the same absolute pixel tolerance that was
  // tuned for ~64px thumbnails. Reference: 0.8px tolerance at 64px source.
  const tolerance = 0.8 * (Math.min(w, h) / 64);
  // Minimum contour span (in source px) to keep — filters texture-speckle
  // noise without discarding genuine small interior linework. Scales down
  // with lower edgeDetailSigma so finer detail isn't pre-filtered out.
  const minSpanPx = Math.max(2, Math.min(w, h) * 0.01 * (edgeDetailSigma / 0.8));
  const polys = [];
  for (const pts of paths) {
    ...
    const span = (bxmax - bxmin) + (bymax - bymin);
    if (span < minSpanPx) continue;  // too small to read at any export scale
    const simplified = simplifyPolyline(pts, tolerance);
    if (simplified.length < 3) continue;
    // significance = perimeter span; used to rank contours, finest last
    polys.push({ pts: simplified.map(p => [p[0] / w, p[1] / h]), sig: span });
  }
  // Sort by significance (largest first: outer silhouette tends to win and
  // becomes polys[0] / "dom"), but keep a much larger pool than before so
  // recognizable interior linework reaches "inner" in the medoid exemplar.
  // Higher detail (lower edgeDetailSigma) -> more contours retained.
  polys.sort((a, b) => b.sig - a.sig);
  const maxKeep = Math.round(6 + 24 * Math.min(1, Math.max(0, (1.5 - edgeDetailSigma) / 1.2)));
  return polys.slice(0, maxKeep).map(o => o.pts);
}
...
function preprocessImage(srcImg) {
  ...
  const pre = gaussianBlurFloat(gray, w, h, edgeDetailSigma);
...
function precomputeAllEdgeMaps() {
  ...
  edgeMapsReady = 0;
  edgeMapsTotal = 0;
  edgeMapsRequested = true;
  let polyTotal = 0;
  for (const p of POINTS) {
    // Prefer the high-resolution trace source (edge_img) so interior detail
    // — mullions, figures, drawn linework, texture — survives the Sobel
    // pass at fine sigma, and so worlds-mode exemplars can carry
    // recognizable content. Falls back to the display sprite if no high-res
    // source was exported for this point.
    const srcB64 = p.edge_img || p.img;
    if (!srcB64) continue;
    edgeMapsTotal++;
    const img = new Image();
    img.onload = () => {
...
function computeSegmentationPaths(processWidth, targetW, targetH) {
  if (edgeMaps.size === 0) {
    // First-ever call (or mid-rebuild) — kick off the async precomputation
    if (!edgeMapsRequested) precomputeAllEdgeMaps();
    return null;
  }
...
with st.expander("Fluid 3D navigation (with image markers)", expanded=True):
    ...
    edge_trace_size = st.slider(
        "Detail tracing source size (px)", 64, 512, 256, 32,
        help="Resolution of the image fed to the segmentation linework's "
             "Sobel edge pass and the worlds-mode content polygons / "
             "exemplars, independent of the on-screen sprite size above. "
             "Higher values let recognizable interior features — mullions, "
             "figures, drawn linework, texture — survive into the worlds "
             "exemplars and the exported linework SVG. Larger values "
             "increase load time and memory, since one image is loaded and "
             "processed per point.",
        key="edge_trace_size",
    )
    ...
    for i in range(len(df_valid)):
        ...
        # High-resolution source for the segmentation linework's Sobel pass
        # and the worlds-mode content polygons / exemplars. Independent of
        # the on-screen sprite (threejs_thumb) so detail tracing can run
        # finer than the display resolution without bloating every sprite's
        # texture memory.
        edge_b64 = None
        try:
            edge_b64, _ = get_thumbnail_b64(row["path"], edge_trace_size)
        except Exception:
            pass
        ...
        sprites_data.append({
            ...
            "exp_ar": exp_ar,
            "edge_img": edge_b64,             # high-res source for detail tracing
